...
<script lang="ts">
    /* 
    New component that unifies the layout and functionality of Add and Edit, bringing 
    the 'app-like' mobile experience flawlessly to desktop.
    */
    ...
    let draftImagePath = item?.photos?.find(p => p.type === 'product')?.cropPath || item?.photos?.find(p => p.type === 'product')?.orgPath || "";
